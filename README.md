...
POST request for evert 2,000 existing records that have been altered within its block. It will return
`false` if Xero reports an error on any of the records, or `true` if they were all saved successfully.
`batch_save` takes one optional argument: the number of records to create/update per request. (Defaults
to 2,000.)
